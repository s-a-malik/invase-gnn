...
class InvaseGNN(nn.Module):
    ...
    def __init__(self, dim, label_dim, args):
        ...
        # Build the selector
        self.actor = Actor(self.dim, self.actor_h_dim, self.n_layer, self.activation)
        # Build the predictor
        self.critic = Critic(self.dim, self.critic_h_dim, self.label_dim, self.n_layer, self.activation)
    ...
    def evaluate(self, generator, criterion, optimizer, device, task="train"):
        ...
        with trange(len(generator)) as t:
            for x_batch, y_batch in generator:
                ...
                if task == "test":
                    ...
                    x_test.append(orig)
                    y_true.append(y_batch.clone().detach())
                    y_pred.append(preds.cpu())
                else:
                    loss = criterion(logits, y_batch)
                    loss_meter.update(loss.data.cpu().item(), y_batch.size(0))
        ...
        if task == "test":
            return test_acc.avg, torch.cat(x_test, 0), torch.cat(y_true, 0), torch.cat(y_pred, 0)
        else:
...
class INVASETrainer:
    """Class that handles training of INVASE.
    """

    def __init__(self, dim, label_dim, args, load_path=None):
        self.dim = dim
        self.label_dim = label_dim
        self.model = InvaseGNN(self.dim, self.label_dim, args)
